...
// ключ позиции — тикер + сторона
const keyOf = (p) => `${p.asset} ${p.side}`;

// ждём появления именно таблицы с позициями и кликаем Perpetual
async function waitPositionsTable(page) {
  // даём странице загрузиться и привестись
  await page.goto(TRADER_URL, { waitUntil: "networkidle2", timeout: 120000 });
  // иногда таблица рендерится лениво — проскроллим
  await sleep(1500);
  await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  await sleep(1500);
  await page.evaluate(() => window.scrollTo(0, 0));

  // клик по Perpetual, если есть
  await page.evaluate(() => {
  ...
  // ждём таблицу с заголовками "Asset" и "Position Value / Size"
...
async function grabPositions(page) {
  const rows = await page.evaluate(() => {
    ...
    return trs.map(tr => {
      ...
      // Asset — первый TD, в нём может быть "ETH\n10x" → берём первую строку
      let assetCell = norm(tds[0]?.innerText || "");
      let asset = assetCell.split(/\n/)[0].trim();

      // Side: ищем LONG/SHORT
      const side = /short/i.test(rowText) ? "SHORT" : "LONG";

      // Ищем размер в монетах — шаблон "123,456.78 ETH", "44,317,789 XRP" и т.п.
      // Ищем именно для найденного тикера:
      let sizeStr = "";
      let size = 0;
      ...
      if (m) {
        ...
        // fallback: ищем любую пару "число + ТИКЕР" в строке
...
function diffPositions(prevKeys, cur) {
  ...
  // изменение размера (мелочь игнорируем)
  const sizeChanged = [];
  for (const p of cur) {
    const k = keyOf(p);
    if (prev.has(k)) {
      ...
      if (abs > SIZE_TOL && rel > SIZE_TOL_REL) sizeChanged.push({ k, prevSize, curSize: p.size, p });
    }
  }
...
(async () => {
  ...
  try {
    ...
    // 1) Дождаться именно таблицы с позициями
    ...
    // 5) Плановый heartbeat раз в N часов
    const now = Date.now();
    ...
    // 6) Сохраняем состояние (ключ → {size})
    const nextKeys = {};
