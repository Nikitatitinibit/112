...
// ручные флаги
const FORCE_HEARTBEAT = process.env.FORCE_HEARTBEAT === "1";
const FORCE_SNAPSHOT  = process.env.FORCE_SNAPSHOT  === "1";
...
async function sendTelegram(text) {
  ...
  try {
    ...
    if (!res.ok) console.error("Telegram send HTTP", res.status, await res.text());
  } catch (e) {
...
async function waitPositionsTable(page) {
  ...
  // иногда SPA дольше отрисовывает контент — помотаем
  await sleep(1500);
  await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  await sleep(1500);
  await page.evaluate(() => window.scrollTo(0, 0));

  // клик по Perpetual, если он есть
  await page.evaluate(() => {
...
(async () => {
  ...
  try {
    ...
    console.log(`ℹ️ Positions found: ${curPositions.length}`);

    const state = loadState();
    const prevKeys = FORCE_SNAPSHOT ? {} : (state.keys || {});
    ...
    if (blocks.length) {
      ...
      console.log("ℹ️ No diffs to report.");
    }

    const now = Date.now();
    const needHeartbeat = FORCE_HEARTBEAT || !state.lastHeartbeat || now - state.lastHeartbeat > HEARTBEAT_HOURS * 3600 * 1000;
    console.log(`ℹ️ Heartbeat? ${needHeartbeat ? "YES" : "no"} (force=${FORCE_HEARTBEAT})`);
